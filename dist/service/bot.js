...
client.on('messageCreate', async (message) => {
    ...
    const researchCategory = message.guild?.channels.cache.find((ch) => ch.type === ChannelType.GuildCategory && ch.name.toLowerCase() === 'research');
    if (!researchCategory) {
    ...
    for (const [channelName, entries] of Object.entries(grouped)) {
        ...
        const channel = message.guild?.channels.cache.find((ch) => ch.isTextBased() &&
        ...
        const groupedByIdentityAndDeadline = entries.reduce((acc, row) => {
